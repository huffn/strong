...
api.pageTranslations = function(options) {
  if (options == null) options = {};
  var locale = this.effective_locale(options);
  var template = this.lookup(locale, options.view_path);
  return template;
};
...
api.decorator = function(engine, appname) {
  ...
  return {
    ...
    render: function (template, options) {
      ...
      var all_translations = function(options) {
        if ( !exists(options) ) options = {};
        ...
        return self.pageTranslations.apply(self, [options]);
      };
      ...
      options.i18n.allLocales = all_translations;
